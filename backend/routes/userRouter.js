...
const PracticeQuestions = require("../models/practiceQuestionsModel");
...
router.post("/startExam", async (req, res) => {
  try {
    ...
    console.log("startExam chala");

    if (!examCode) return res.status(400).json({ msg: "Exam code invalid." });
    console.log(examCode);
    const exam = await Exam.findOne({ examCode });
    console.log("Exam", exam);
    if (!exam) return res.status(400).json({ msg: "No exam with this examCode has been registered." });
    res.json({
      examCode,
...
/*router.post("/practiceQuestions", async (req, res) => {
    try
    {
        console.log("\n\n\n\n\nPractice questions chala\n\n\n\n\n");
        const data = await PracticeQuestions.find(function (err, data) {
